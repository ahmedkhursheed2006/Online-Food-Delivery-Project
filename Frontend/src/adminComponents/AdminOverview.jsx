...
function AdminOverview() {
  ...
  const [filteredUsers, setFilteredUsers] = useState(userData.admin);

  const { signup } = useAdminStore();

  const initialFormState = adminForm.reduce((acc, input) => {
    acc[input.name] = input.type === "radio" ? "" : "";
    return acc;
  }, {});
  ...
  useEffect(() => {
    if (searchTerm === "") {
      setFilteredUsers(userData.admin);
    } else {
      const results = userData.admin.filter((admin) => {
  ...
  return (
    <div className="relative p-6 bg-white/99 rounded-lg shadow-md w-full h-screen flex flex-col">
      <header className="text-right flex justify-between items-center">
        ...
        <button
          className="text-[#1566E5] text-xl p-1 font-semibold border-[#666] border-2 rounded-full"
      ...
      >
        ...
        <button type="submit" className="bg-[#1566E5] text-white w-full text-lg">Submit</button>
      </form>
    </div>
  );
}
